...
async function renderReport() {
  ...
  const cliData = state.clienti.map(cl=>({...cl,n:ordiniReport.filter(o=>o.clienteId===cl.id).length})).sort((a,b)=>b.n-a.n);
  ...
  const prodFreq = {}; ordiniReport.forEach(o=>o.linee.forEach(l=>{if(l.prodId)prodFreq[l.prodId]=(prodFreq[l.prodId]||0)+(l.qty||1);}));
  ...
  const giroFreq = {}; ordiniReport.forEach(o=>{const g=getCliente(o.clienteId)?.giro||'altro';giroFreq[g]=(giroFreq[g]||0)+1;});
  ...
  const clientiInattivi = state.clienti.map(cl=>{const oc=ordiniReport.filter(o=>o.clienteId===cl.id);const ul=oc.length?oc.map(o=>o.data).sort().reverse()[0]:null;return{...cl,ultimo:ul};}).filter(cl=>!cl.ultimo||cl.ultimo<sogliaStr).sort((a,b)=>(a.ultimo||'').localeCompare(b.ultimo||'')).slice(0,15);
  ...
  const agCliData = agenti.map(a=>({agente:(a.nome+' '+(a.cognome||'')).trim(),clienti:state.clienti.filter(cl=>cl.agenteId===a.id)}));
