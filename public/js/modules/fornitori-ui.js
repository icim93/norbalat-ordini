(function () {
  ...
  function defaultEmailBody(order = null) {
    ...
    const note = order?.noteOrdine || document.getElementById('ordine-fornitore-note-ordine')?.value || '';
    return [
      'Buongiorno,',
      '',
  ...
  function syncFornitoreFromProdotto() {
    const prodottoInput = document.getElementById('ordine-fornitore-prodotto');
    const fornitoreSelect = document.getElementById('ordine-fornitore-fornitore');
    if (!prodottoInput || !fornitoreSelect) return;
    const prodotto = getProdottoByInput(prodottoInput.value);
    if (!prodotto?.fornitoreRiferimentoId) return;
    if (!fornitoreSelect.value) {
      fornitoreSelect.value = String(prodotto.fornitoreRiferimentoId);
    }
    const emailInput = document.getElementById('ordine-fornitore-email-to');
    if (emailInput && !emailInput.value.trim()) {
      const fornitore = getFornitore(prodotto.fornitoreRiferimentoId);
      if (fornitore) emailInput.value = emailListToText(getFornitoreDefaultEmails(fornitore));
    }
  }
  ...
  function syncOrdineFornitoreRecipients() {
    ...
    if (fornitore) input.value = emailListToText(getFornitoreDefaultEmails(fornitore));
  }

  function collectOrdineFornitorePayload() {
    const isWarehouse = !canManageOrdiniFornitori();
    const prodottoInputRaw = String(document.getElementById('ordine-fornitore-prodotto')?.value || '').trim();
    const prodotto = getProdottoByInput(prodottoInputRaw);
    const prodottoId = prodotto?.id || null;
    const prodottoNome = prodotto ? prodotto.nome : prodottoInputRaw.replace(/^\[[^\]]+\]\s*/, '');
    if (!prodottoNome) {
      window.showToast('Seleziona un prodotto', 'warning');
      return null;
    }
    if (isWarehouse) {
      if (!prodottoId) {
        window.showToast('Seleziona un prodotto dall\'elenco', 'warning');
        return null;
      }
      if (!prodotto?.fornitoreRiferimentoId) {
        window.showToast('Il prodotto selezionato non ha un fornitore di riferimento impostato', 'warning');
        return null;
      }
      return {
        fornitore_id: prodotto.fornitoreRiferimentoId,
    ...
    const quantita = Number(String(document.getElementById('ordine-fornitore-quantita')?.value || '0').replace(',', '.'));
    if (!fornitoreId) {
      window.showToast('Seleziona un fornitore', 'warning');
      return null;
    }
    return {
      ...
      prodotto_id: prodottoId,
      prodotto_nome: prodottoNome,
      quantita: Number.isFinite(quantita) && quantita >= 0 ? quantita : 0,
      unita_misura: String(document.getElementById('ordine-fornitore-unita')?.value || '').trim(),
      note_magazzino: String(document.getElementById('ordine-fornitore-note-magazzino')?.value || '').trim(),
  ...
  function updateOrdineFornitoreModalMode(order = null) {
    ...
    const waBtn = document.getElementById('ordine-fornitore-wa-btn');
    ...
    const fornitoreRow = document.getElementById('ordine-fornitore-fornitore-row');
    const quantitaRow = document.getElementById('ordine-fornitore-quantita-row');
    if (emailFields) emailFields.style.display = manager ? '' : 'none';
    if (dirFields) dirFields.style.display = manager ? '' : 'none';
    if (fornitoreRow) fornitoreRow.style.display = manager ? '' : 'none';
    if (quantitaRow) quantitaRow.style.display = manager ? '' : 'none';
    if (sendBtn) sendBtn.style.display = manager && !sent ? '' : 'none';
    if (waBtn) waBtn.style.display = manager && !sent ? '' : 'none';
    if (saveBtn) {
      saveBtn.style.display = sent || readonlyForWarehouse ? 'none' : '';
      saveBtn.textContent = manager ? 'Salva formulazione' : 'Invia richiesta';
    }
  ...
  function openOrdineFornitoreModal(id = null) {
    ...
    document.getElementById('modal-ordine-fornitore-title').textContent = order ? `Ordine fornitore #${order.id}` : 'Nuova richiesta ordine';
    document.getElementById('modal-ordine-fornitore-subtitle').textContent = order
      ...
      : (canManageOrdiniFornitori()
          ? 'Compila tutti i campi e invia l\'ordine al fornitore.'
          : 'Seleziona il prodotto: il fornitore verrà collegato automaticamente.');
    ...
    document.getElementById('ordine-fornitore-prodotto').value = order
      ...
      : '';
    document.getElementById('ordine-fornitore-quantita').value = order?.quantita || '';
  ...
  async function persistOrdineFornitore({ close = true } = {}) {
    ...
    try {
      ...
      if (close) {
        ...
        window.showToast('Richiesta salvata', 'success');
      }
      return normalized;
    } catch (e) {
      window.showToast(e.message || 'Errore salvataggio', 'warning');
      return null;
    }
  }
  ...
  function sendOrdineFornitoreWhatsApp() {
    if (!canManageOrdiniFornitori()) return;
    const fornitoreId = document.getElementById('ordine-fornitore-fornitore')?.value;
    const fornitore = getFornitore(fornitoreId);
    const rawPhone = String(fornitore?.telefono || '').replace(/\s/g, '');
    if (!rawPhone) {
      window.showToast('Nessun numero di telefono per questo fornitore', 'warning');
      return;
    }
    const digits = rawPhone.replace(/\D/g, '');
    const intlPhone = digits.startsWith('39') ? digits : (digits.startsWith('0') ? `39${digits.slice(1)}` : `39${digits}`);
    const body = String(document.getElementById('ordine-fornitore-email-body')?.value || '').trim()
  ...
  async function saveFornitore() {
    ...
    try {
      ...
      if (typeof window.renderAnagraficaFornitoriPage === 'function') window.renderAnagraficaFornitoriPage();
      renderFornitoriPage();
  ...
  function renderOrdiniFornitoriTable() {
    ...
    tbody.innerHTML = rows.length ? rows.map(o => `
      <tr>
        ...
        <td style="font-family:'DM Mono',monospace;white-space:nowrap;">${o.quantita ? `${formatQty(o.quantita)} ${window.escapeHtml(o.unitaMisura || '')}` : '<span style="color:var(--text3);">—</span>'}</td>
  ...
  function renderFornitoriList() {
    ...
    if (q) rows = rows.filter(f => [f.nome, f.localita, f.email, f.pec, f.contattoNome, f.telefono].join(' ').toLowerCase().includes(q));
      ...
      <div style="border:1px solid var(--border);border-radius:8px;padding:10px;background:var(--surface);">
        <div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;">
          <div style="min-width:0;">
            ...
            ${f.telefono ? `<div style="font-size:11px;color:var(--text3);">${window.escapeHtml(f.telefono)}</div>` : ''}
          </div>
  ...
  async function renderAnagraficaFornitoriPage(options = {}) {
    if (!canManageFornitori()) return;
    if (options.load) {
      try {
        await window.api('GET', '/api/fornitori').then(list => {
          window.state.fornitori = (list || []).map(window.normalizeFornitore);
        });
      } catch (e) {
        window.showToast(e.message || 'Errore caricamento fornitori', 'warning');
      }
    }
    renderFornitoriList();
  }
  ...
  window.sendOrdineFornitoreWhatsApp = sendOrdineFornitoreWhatsApp;
  ...
  window.renderAnagraficaFornitoriPage = renderAnagraficaFornitoriPage;
  window.syncOrdineFornitoreRecipients = syncOrdineFornitoreRecipients;
  window.syncFornitoreFromProdotto = syncFornitoreFromProdotto;
