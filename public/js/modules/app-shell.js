(function () {
  ...
  const NAV_ICONS = {
    ...
    domani: `<svg ${S}><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/><path d="M10 14l3 3-3 3"/></svg>`,
  ...
  const mobileNavConfigs = {
    admin: ['dashboard', 'ordini', 'magazzino', 'giacenze'],
  ...
  function getNavBadgeValue(page) {
    ...
    if (page === 'riepilogo-domani') {
      const d = new Date();
      d.setDate(d.getDate() + 1);
      const pad = v => String(v).padStart(2, '0');
      const tomorrow = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
      return ordini.filter(o => o.data === tomorrow && o.stato !== 'annullato').length;
    }
    if (page === 'magazzino') {
  ...
  function renderPage(page) {
    ...
    if (page === 'riepilogo-domani' && typeof window.renderRiepilogoDomani === 'function') window.renderRiepilogoDomani();
